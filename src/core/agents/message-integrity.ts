...
export type MessageActor = MessageAssignment['actor']
export type AuthenticateRecipient = () => Promise<MessageActor | null>
export const sameMessageActor = (left: MessageActor | null, right: MessageActor): boolean =>
  !!left && (['agent_id', 'node', 'pane', 'session_id', 'incarnation', 'provider'] as const)
...
export class MessageIntegrity {
  ...
  /** A host authenticator, not a caller-supplied actor or a generic turn hook, proves receipt. */
  async acknowledge(req: IdentifiedMessage, status: 'acknowledged' | 'accepted',
    authenticate?: AuthenticateRecipient): Promise<AgentMessageOutcome> {
    return this.acknowledgeKnown(req, status, authenticate, this.receipt(req))
  }

  /** Recipient references only the app-authored envelope IDs, never reconstructs unsanitized
   * body/verb bytes or supplies a replacement assignment. The existing cache is the only store. */
  async acknowledgeReceipt(reference: { sourceNodeId: string; message_id: string; action_id: string },
    status: 'acknowledged' | 'accepted', authenticate?: AuthenticateRecipient): Promise<AgentMessageOutcome> {
    const entry = this.receipts.get(JSON.stringify([reference.sourceNodeId, reference.message_id]))
    if (!entry || entry.message.action_id !== reference.action_id)
      return { kind: 'unknown', reason: 'receipt-unavailable' }
    return this.acknowledgeKnown({ sourceNodeId: entry.source, targetNodeId: entry.message.actor.node,
      body: '', message: entry.message }, status, authenticate,
    entry.outcome ?? { kind: 'unknown', reason: 'receipt-unavailable' })
  }

  private async acknowledgeKnown(req: IdentifiedMessage, status: 'acknowledged' | 'accepted',
    authenticate: AuthenticateRecipient | undefined, previous: AgentMessageOutcome): Promise<AgentMessageOutcome> {
    if (!authenticate || !req.message) return rejected('recipient-authentication-unavailable')
    const snapshot = structuredClone(req)
    try {
      if (!sameMessageActor(await authenticate(), snapshot.message!.actor)) return rejected('recipient-mismatch')
    } catch { return rejected('recipient-authentication-unavailable') }
    req = snapshot
    ...
    // Recheck the authenticated incarnation after the asynchronous canonical read. These remain
    // cooperative snapshots, not an atomic lock across two independent authorities.
    try {
      if (!sameMessageActor(await authenticate(), req.message.actor)) return rejected('recipient-mismatch')
    } catch { return rejected('recipient-authentication-unavailable') }
    // Authentication may itself await I/O: a transfer during it must be seen before receipt.
    // No further await separates this canonical snapshot (and its TTL check) from publication.
    const finalRefusal = await this.guard(req, 'acknowledgment')
    if (finalRefusal) return this.attach(req.message, finalRefusal)
    const entry = this.receipts.get(key(req))!
