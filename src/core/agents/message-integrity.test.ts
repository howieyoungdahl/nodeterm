...
const authenticate = async () => request().message!.actor
...
describe('correlated queued messages', () => {
  it('deduplicates concurrent retries and distinguishes delivery, acknowledgment and work acceptance', async () => {
    ...
    expect((await f.queue.messages.acknowledge(req, 'acknowledged', authenticate)).message?.status).toBe('acknowledged')
    expect((await f.queue.messages.acknowledge(req, 'accepted', authenticate)).message?.status).toBe('accepted')
    expect((await f.queue.messages.acknowledge(req, 'acknowledged', authenticate)).message?.status).toBe('accepted')
  ...
  it('fences queued delivery and late recipient acceptance after transfer', async () => {
    ...
    g.setEpoch(2)
    expect(await g.queue.messages.acknowledge(req, 'accepted', authenticate)).toMatchObject({ kind: 'messageRejected', reason: 'stale_epoch' })
