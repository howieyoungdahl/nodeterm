...
describe('local save rescue', () => {
  it('rejects a failed project write, preserves the old cards, and saves the latest snapshot on retry', async () => {
    ...
    store.registerIpc()
    // Legacy internal producers retain failure propagation; IPC writes require enrolled bases.
    const save = (workspace: Workspace) => store.save(workspace, { client: 'ui:7' })
  ...
  it('refuses unenrolled legacy IPC proposals without rescuing or changing existing cards', async () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => undefined)
    const fake = fakePlatform({ userDataDir: userData })
    initPlatform(fake)
    const store = new WorkspaceStore(undefined, guards({ hasLiveBackend: () => true }))
    store.registerIpc()
    await store.save(ws([project({ cwd: projRoot })]))
    await expect(fake.handlers['workspace:save'](ws([project({ cwd: projRoot, nodes: [node('term-1')] })]))).rejects.toThrow('E_EXPECTED_REVISION_REQUIRED')
    expect(await readNodes(projRoot)).toEqual(['term-1', 'term-2'])
    expect(warn.mock.calls.map((c) => String(c[0])).join('\n')).not.toContain('[workspace] rescued')
  })
