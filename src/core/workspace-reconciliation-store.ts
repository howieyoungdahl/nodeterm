...
import { ProjectCommitStore, revisionOf, type FileRevision, type FileCommitResult } from './project-commit-store'
...
interface WorkspaceDriver {
  ...
  entry(id: string): IndexEntryV3 | undefined
...
export class WorkspaceReconciliationStore {
  ...
  async load(clientId?: string): Promise<WorkspaceRevisionView> {
    ...
    this.clients.set(id, client)
    const workspace = await this.driver.load()
    const projects: WorkspaceRevisionView['projects'] = {}
    const unsupported: string[] = []
    for (const project of workspace.projects) {
      const entry = this.driver.entry(project.id)
      if (!entry || project.ssh || project.unavailable || project.remote) { unsupported.push(project.id); continue }
    ...
    let indexRevision = ''
    try {
      const snapshot = await new ProjectCommitStore(this.driver.indexPath, undefined, indexKeys).observe()
    ...
    } catch { /* Legacy/absent indexes require explicit migration before versioned mutation. */ }
