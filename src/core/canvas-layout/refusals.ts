...
import type { LayoutSkipReason } from '../../shared/canvas-layout'
...
export function refusalContext(input: LayoutInput): RefusalContext {
  return {
...
export function refuse(node: LayoutNode, ctx: RefusalContext): LayoutSkipReason | null {
  ...
  if (!isWorkerNode(node)) return 'primary-role'
  return null
}
