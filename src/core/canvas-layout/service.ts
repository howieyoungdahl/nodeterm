...
import { OrganizerCoordinator, type OrganizerRuntime } from './coordinator'
import type { WorkspaceReconciliationStore } from '../workspace-reconciliation-store'
import type { LayoutCommitRequest } from '../../shared/canvas-layout'
...
export interface CanvasLayoutServiceDeps {
  ...
  now?: () => number
  store?: WorkspaceReconciliationStore
...
export function registerCanvasLayoutIpc(deps: CanvasLayoutServiceDeps = {}): void {
  const lease = deps.lease ?? new LayoutLeaseStore({ now: deps.now })
  const coordinator = deps.store ? new OrganizerCoordinator({ store: deps.store, lease, runtime: deps.runtime,
    enabled: () => layoutEngineEnabled(sanitizeCanvasLayoutSettings(deps.settings?.())), now: deps.now,
    plan: (request) => planForRequest(request, { ...deps, lease }) }) : undefined
  platform().handleWithSender(IPC.canvasLayoutPlan, (senderId, request: LayoutPlanRequest) => coordinator
    ? coordinator.prepare(senderId, request)
    : { ...standDownPlan(request?.trigger ?? 'organize', 'source-unavailable'), refusal: 'coordinator-unavailable' })
  for (const [channel, inverse] of [[IPC.canvasLayoutApply, false], [IPC.canvasLayoutInverse, true]] as const) {
    platform().handleWithSender(channel, (senderId, request: LayoutCommitRequest) => coordinator
      ? coordinator.commit(senderId, request, inverse)
      : { kind: 'refused', operationId: request?.operationId, reason: 'coordinator-unavailable' })
  }
  platform().handle(IPC.canvasLayoutRelease, async (payload: unknown) => {
    const { projectId, holder, leaseToken } = (payload ?? {}) as { projectId?: string; holder?: string; leaseToken?: string }
    if (!projectId || !holder || !leaseToken) return false
    const current = lease.holder(projectId)
    if (current?.holder !== holder || current.token !== leaseToken) return false
    await lease.release(projectId, holder, leaseToken)
    return true
  })
}
