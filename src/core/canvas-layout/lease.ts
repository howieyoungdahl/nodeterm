...
export class LayoutLeaseStore {
  ...
  /** A coordinator publication holds the same lease lock through its awaited durable receipt.
   * The caller must recheck TTL/evidence at each publication guard; this is not an idle grant. */
  async withPublication<T>(projectId: string, holder: string, token: string,
    run: (check: () => string | undefined) => Promise<T>): Promise<T> {
    return this.exclusive(async () => {
      const check = (): string | undefined => {
        const held = this.holder(projectId)
        return held?.holder === holder && held.token === token ? undefined : 'lease-stale'
      }
      const refusal = check()
