...
describe('canvas-layout channel parity', () => {
  it('the DESKTOP shell registers it, reading the machine-local switch', () => {
    ...
    expect(src).toMatch(
      /registerCanvasLayoutIpc\(\{\s*settings:\s*\(\)\s*=>\s*settingsStore\.get\(\)\.canvasLayout,\s*store:\s*workspaceStore\.organizerCoordinator\(\)\s*\}\)/
    )
  })
  ...
  it('the preload exposes it on the desktop api', () => {
    ...
    expect(src).toContain('IPC.canvasLayoutApply')
    expect(src).toContain('IPC.canvasLayoutInverse')
  })
