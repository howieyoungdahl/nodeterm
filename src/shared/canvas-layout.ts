...
export interface LayoutPlan {
  /** Opaque host-retained operation. The returned ops are preview data, never apply authority. */
  operationId?: string
  leaseHolder?: string
  baseRevision?: string
 ...
 *
 * The renderer describes its view, but the registered transaction route plans from its exact
 * coordinator-enrolled base and trusted runtime evidence. Working-input counters include edits
 * followed by undo. Unsaved/stale views cannot become authority merely by sending current nodes.
 */
export interface LayoutPlanRequest {
  binding?: { clientId: string; revision: string; inputRevision: string }
  projectId: string
  trigger: LayoutTrigger
  nodes: LayoutNode[]
...
export interface LayoutCommitRequest {
  /** Settlement never publishes; safe after the working input, opt-in or lease changed. */
  settleOnly?: boolean
  operationId: string
  projectId: string
  clientId: string
...
export interface LayoutCommitOutcome {
  kind: 'committed' | 'already-applied' | 'refused' | 'unknown'
  operationId: string
  reason?: string
  current?: import('./workspace-reconciliation').ProjectRevisionView
  recovery?: string
}
