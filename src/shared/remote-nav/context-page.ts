 ...
 */
export type ContextOperation = 'overview' | 'task' | 'owner' | 'attention' | 'handoff' | 'changes'
...
export type ContextPage =
  ...
  | { ok: true; code: 'ok'; operation: ContextOperation; scope: ContextScope;
...
export function adaptContextPage(payload: unknown, request: ContextRequest): ContextPage {
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 100 || !Number.isInteger(ceiling) ||
      ...
      !['overview', 'task', 'owner', 'attention', 'handoff', 'changes'].includes(request.operation) ||
  ...
  if (payload.code !== 'ok' || payload.operation !== request.operation || !record(payload.scope) ||
  ...
  const result: ContextPage = { ok: true, code: 'ok', operation: request.operation, scope: { ...request.scope },
