...
describe('bounded context page consumer', () => {
  ...
  it.each(['continue', 'read_limit'])('retains D15 successful partial %s pages', (code) => {
    const p = { ...page(), code, records: [], truncated: true, continuation: { offset: 900 } }
    expect(adaptContextPage(p, request)).toMatchObject({ ok: true, code, records: [], continuation: { offset: 900 } })
    expect(adaptContextPage({ ...p, truncated: false, continuation: null }, request)).toMatchObject({ code: 'corrupt' })
  })
