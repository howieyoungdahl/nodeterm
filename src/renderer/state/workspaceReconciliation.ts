...
export class WorkspaceReconciliationClient {
  ...
  private organizerBusy = false
  readonly conflicts = new Map<string, ProjectConflict[]>()
  error: string | null = null
  constructor(private api: WorkspaceApi) {}

  organizerBinding(projectId: string, inputRevision: string) {
    const base = this.bases.get(projectId)
    if (!this.view || !base || this.pending || this.inFlight || this.organizerBusy || this.conflicts.has(projectId)) return undefined
    return { clientId: this.view.clientId, revision: base.revision, inputRevision }
  }

  beginOrganizer(): boolean {
    if (this.pending || this.inFlight || this.organizerBusy) return false
    this.organizerBusy = true; this.epoch++
    return true
  }
  endOrganizer(): void { this.organizerBusy = false; this.epoch++ }

  /** Adopt only a matching durable organizer receipt, merging edits made while awaiting it. */
  acceptOrganizer(projectId: string, proposed: Project, current: Workspace,
    result: import('@shared/canvas-layout').LayoutCommitOutcome): Workspace {
    if ((result.kind !== 'committed' && result.kind !== 'already-applied') || !result.current) return current
    const next = result.current
    this.epoch++
    const projects = current.projects.map((local) => local.id === projectId
      ? this.merge(projectId, proposed, local, next.project) : local)
    this.bases.set(projectId, structuredClone(next))
  ...
  async refresh(current: () => Workspace, retry = true): Promise<Workspace> {
    ...
    const projects = workspace.projects.map((local) => {
      ...
      if (this.pending || this.organizerBusy) {
        this.refreshOwed = true
  ...
  async save(workspace: Workspace, current: () => Workspace): Promise<{ workspace: Workspace; saved: boolean }> {
    if (this.organizerBusy) throw new Error('Organizer publication is in flight; retry this save after its receipt.')
