...
- **The status badge (six states, and where each one may come from)** — `shared/node-status.ts` is
  ...
  **Never infer state from what a terminal looks like.** No output parsing, no "it has not printed
  ...
  - `failed` comes ONLY from a session fact: the last hook state was `working` **and** the pane is
    PROVEN dead. The proof is `PtyManager.sessionPresence` — the same tri-state primitive
    `ServerNodeOps`' dead-card sweep asks — served by `core/node-status-service.ts`, which repeats
    the probe before it will answer `dead` (one miss is a busy tmux, two is a fact). Both shells
    register that channel from the one core body; `core/node-status-parity.test.ts` pins it,
    because a shell that stopped calling it would boot fine and simply never say `failed`.
  ...
  Probing is lazy: `paneProbeCandidates` picks only `working` entries past `NODE_STATUS_STALE_MS`
  (10 min — half the mirror's 20-minute presumed-gone window, and just past the longest legitimate
  gap between hook events), so a canvas of healthy agents makes zero probes. Freshness is rendered
